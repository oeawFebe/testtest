...
def quickwrite():
    ...
    # time.
    if os.path.exists(os.path.join(settings.BASE_DIR,'info','token.json')):
        creds = Credentials.from_authorized_user_file(os.path.join(settings.BASE_DIR,'info','token.json'), SCOPES)
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                os.path.join(settings.BASE_DIR,'info','credentials.json'), SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open(os.path.join(settings.BASE_DIR,'info','token.json'), 'w') as token:
